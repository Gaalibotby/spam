...
TELEGRAM_BOT_TOKEN = '7774369692:AAHHmFoEjqKq2cgTCQf4stOhChL03iOh7Ns'
...
async def attack(update: Update, context: CallbackContext):
    ...
    asyncio.create_task(run_attack(chat_id, ip, port, time, context, msg))

# Run attack function (edits the same message)
async def run_attack(chat_id, ip, port, time, context, msg):
    global attack_in_progress
    ...
    try:
        ...
        # Countdown timer (updates the same message every second)
        while time > 0:
            try:
                await msg.edit_text(f"⏳ Attack in progress... {time} sec remaining", parse_mode='Markdown')
            except Exception:
                pass  # Ignore edit errors if the message was deleted or cannot be edited
